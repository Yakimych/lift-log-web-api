...
let getCollection<'a> (mongoDbSettings: Settings) =
    let client = MongoClient(mongoDbSettings.mondoDbServer)
...
let getAll (mongoDbSettings: Settings): LiftLog seq =
    getCollection<LiftLogObject>(mongoDbSettings).Find(fun l -> true).ToList() |> Seq.map toLiftLog
